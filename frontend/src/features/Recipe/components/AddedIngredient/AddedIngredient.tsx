...
import { updateRecipeField, addedIngredient } from '../../store/recipeSlice';
...
export const AddedIngredient: React.FC<AddedIngredientProps> = ({
  ...
  const [activeIngredientId, setActiveIngredientId] = useState<number | null>(
    null,
  );
  ...
  const handleQuantityConfirm = (
    ...
    dispatch(addedIngredient({ id: id, name: name, amount: quantity }));
        ...
        <ModalInfo
          ingredient={
            listIngredients.find(
              (ingredient) => ingredient.id === activeIngredientId,
            ) || listIngredients[0] // Или другой дефолтный ингредиент, если не найден
          }
          className={styles['ingredients-list__item--modal']} // Изменим класс
        />
      )}
      {!addNewIngredient ? (
        <div className={styles['content-container']}>
                ...
                >
                  ...
                  {selectedIngredient?.id === ingredient.id && (
                    <AmountOfIngredient
                      ...
                      className={
                        styles['ingredients-list__item--quantity-modal']
                      }
                    />
                  )}
                </li>
          ...
          >
            <img
              src={Icons.iconAddedNewIngredient}
              alt="добавить свой ингредиент "
            />
          </button>
        </div>
      ) : (
