...
class UserRepositoryImpl(UserRepository):
    ...
    async def _to_db(self, domain_user: DomainUser) -> DBUser:
        ...
        if domain_user.id is not None:
            ...
            if db_user:
                ...
                db_user.is_admin = domain_user.is_admin
    ...
    async def get_all(self) -> List[DomainUser]:
        ...
        Returns:
            List[DomainUser]: Возвращает список обьектов пользователей.

        """
        result = await self.db_session.execute(select(DBUser))
        db_users = result.scalars().all()
        return [await self._to_domain(ing) for ing in db_users]
